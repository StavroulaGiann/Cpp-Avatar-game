...
Timer::Timer(float max_time, float period, timer_type_t type)
	: m_max(max_time), m_period(period), m_type(type)
{
}
...
Timer::operator float()
{
	if (m_running && !m_pause)
	{
		...
		{
		case TIMER_ONCE:
			m_val = std::min(m_max, (graphics::getGlobalTime() / 1000.f - m_time_start) / m_period);
			if (m_val == m_max)
			{
				m_running = false;
				m_pause = false;
